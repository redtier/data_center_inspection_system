...
if username_value=="cookie不存在": #如果没有获取到username的cookie值则返回登录界面
  ...
  print('</html>')
	...
	#定义空字符串sql_string
	sql_string=""
	#将所有数据写入字符串sql_string，使得sql_string形如"column1 = 'value1'，column2 = 'value',..."
	...
	#将所有数据更新进表form_record的条目（已经创建的巡检表），该条目的formname值与cookie的formname的值一致
	...
	print('<title>保存确认</title>')
	print('</head>')
	print('<body>')
	print('<div align="center">')
	print('<tr>')
	print('<td colspan="11"><div align="center">数据保存成功。</div></td>')
	print('<td colspan="11"><div align="center">1秒后自动返回。</div></td>')
	print('</tr>')
	#数据提交成功后仍然保持在该页面，并延迟600ms返回
	#print('<script language="javascript">')
	#print('setTimeout("window.history.go(-1)",1000);')
	#print('</script>')
	print('</body>')
	print('</html>')