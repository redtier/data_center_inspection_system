...
if auditor_username_value=="cookie不存在": #如果没有获取到username的cookie值则返回登录界面
	...
	#将formname对应的submit_state改为"0"
	ms.ExecNonQuery("UPDATE form_record SET submit_state='0' , auditor_username='"+str(auditor_username_value)+"' , form_audit_date_time='"+str(form_audit_date_time)+"' WHERE formname = "+"'"+str(audit_formname_value)+"'")
	print("Content-type:text/html;charset=gbk")
	print()
	print('<html xmlns="http://www.w3.org/1999/xhtml">')
	print('<head>')
	...
	print('<td colspan="11"><div align="center">巡检表'+str(audit_formname_value)+'退回修改，巡检表状态改为未提交。</div></td>')
